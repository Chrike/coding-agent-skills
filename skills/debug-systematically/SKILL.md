...
Make unclear failures observable, then test grounded causes one at a time.

## First Decision

Apply the Kernel's ordinary-work boundary: obvious compile errors, typos, missing imports, and direct one-line failures take a narrow fix and focused check, not this workflow.
...
Use this as a diagnostic decision loop; apply the Kernel's evidence and stop rules, and skip only diagnostic steps whose decision is already settled by reliable evidence.
...
7. **Take the supported narrow win.** When one hypothesis supports a reversible fix, implement and test it rather than exhausting every remaining hypothesis.
8. **Fix the root cause.** Avoid bundled refactors and symptom patches.
9. **Verify and clean up the diagnosis.** Re-run the original signal, add or keep a regression check when there is a correct seam, and remove temporary instrumentation.
...
If no signal exists, state what was tried and request the missing artifact: repro steps, logs, HAR/network capture, failing input, timestamped screen recording, access to the reproducing environment, or permission for temporary instrumentation. Do not present a confident fix without evidence.
