...
- resolve the project root, candidate parent directory, and target path before writing
- reject a candidate whose resolved path is outside the project root
- reject symbolic links, junctions, or equivalent path indirection unless containment and ownership can be established
- verify the target location is ignored before writing
- create a uniquely named target for the current workflow
- create the target with exclusive new-file semantics when the host supports it
...
- if path containment, target identity, or ownership cannot be established, do not create or remove the handoff; use controller context or host-managed state instead
...
- immediately before cleanup, re-resolve containment, target identity, and workflow ownership; do not rely on the original path check if the path may have changed
...
- retain the created path, workflow-generated unique identifier, and creation result in controller state when needed for cleanup; do not create durable ownership state solely for this
