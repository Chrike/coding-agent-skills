...
The current script invokes the workflow `agent(prompt, options)` API with task-specific inline prompts and schemas. It does not select `.claude/agents/fresh-completion-verifier.md`. That checked-in Agent definition is also the project-scoped runtime target and is independently callable with `claude --agent fresh-completion-verifier`; this repository has no separate top-level Agent mirror or installer. It has not confirmed a supported saved-Agent selector for the workflow API, so keep the inline investigator and verifier fallback until that interface is directly confirmed.
