...
The current script invokes the workflow `agent(prompt, options)` API with task-specific inline prompts and schemas. The current Workflow runtime accepts an `agentType` option, and a live probe verified dispatch of the built-in `Explore` type. A probe with `agentType: "fresh-completion-verifier"` failed before worker execution because that project custom Agent was not in the Workflow registry for the run. It therefore does not yet confirm project `.claude/agents/` loading through Workflow. The checked-in Agent definition remains the project-scoped runtime target and is independently callable with `claude --agent fresh-completion-verifier`; this repository has no separate top-level Agent mirror or installer. Keep the inline investigator and verifier fallback until project saved-Agent loading is directly confirmed.
