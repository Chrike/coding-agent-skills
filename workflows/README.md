...
The current script invokes the workflow `agent(prompt, options)` API with task-specific inline prompts and schemas. A version-specific local Workflow probe accepted an `agentType` option and dispatched the built-in `Explore` type; a probe with `agentType: "fresh-completion-verifier"` failed before worker execution because that project custom Agent was not in that run's Workflow registry. These are runtime observations, not confirmation of a documented or stable public Workflow selector contract; current public Workflow documentation (https://code.claude.com/docs/en/workflows) shows generic `agent(prompt, options)` examples but does not establish `agentType` or project `.claude/agents/` loading. The checked-in Agent definition remains the project-scoped runtime target and is independently callable with `claude --agent fresh-completion-verifier`; this repository has no separate top-level Agent mirror or installer. Keep the inline investigator and verifier fallback until project saved-Agent loading is directly confirmed.
