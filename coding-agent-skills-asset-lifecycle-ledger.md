...
- 日期：2026-07-15
- 阶段：阶段 2 评测基础已启动；Kernel 与 13 个 playbook 的运行时薄化切片已实施并提交；readiness 与 blocked/invalid metadata conformance 自检已建立
- 状态：资产映射、协议、静态 seed、conformance check、Kernel 压缩与 13 个 playbook 局部压缩已建立；readiness self-check 与 blocked/invalid contract self-check 已通过；真实 runner/物理迁移尚未开始
- 当前基线：`full-suite-v1` → `9a0318a`
- 当前分支：`refactor/kernel-playbooks-thin`
- 最新 checkpoint：`589b3cf`（`Add blocked-invalid evaluation conformance`）；前置 checkpoint 为 `fa0b749`
...
| `eval.readiness-slice.test-pos-001` | `tests/evals/scn-tm-test-pos-001-readiness.json`、`scn-tm-test-pos-001-readiness-fixture.js`、`scn-tm-test-pos-001-readiness-oracle.js`、`verify-scn-tm-test-pos-001-readiness.js` | non-runtime readiness self-check | Outcome Evals（readiness only） | `tests/evals/scn-tm-test-pos-001-readiness.json` + independent oracle module | `baseline-validation` | fixture/oracle hash、静态 seed 引用、6 条 readiness criteria、重复稳定性和 fixed-sleep negative control 通过；无 model/host/A-B/outcome claim | 在可核验 host execution substrate、task harness 和 source-revision 对齐后，才考虑把场景升级为 executable scenario；当前不改变 static seed 或 outcome 状态 |
| `eval.blocked-invalid-semantics-v1` | `tests/evals/blocked-invalid-semantics.json`、`tests/evals/verify-blocked-invalid-semantics.js` | non-runtime metadata-only conformance | Outcome Evals（terminal semantics only） | `tests/evals/outcome-protocol.md` §5.4.1/§5.5 + synthetic manifest/verifier | `baseline-validation` | 9 synthetic records、7 paired sets；blocked/invalid reason code、attempt boundary、metric wrapper、terminal pointer、preservation、paired propagation、raw/paired denominator 和 attrition 通过；`outcome_denominator: 0`；无 runtime/A-B/outcome claim | 在可核验 host execution substrate、immutable task harness、model/provider matrix 和 telemetry 具备后，才考虑真实 run collection；当前不改变 static seed、readiness 状态或生命周期 |

当前没有 `tests/scenarios/`、outcome runner、模型矩阵、成本采集器或自动 promotion/demotion/archive 系统。新增的 readiness slice 只验证一个独立 fixture/oracle 的确定性自检，不改变 `scenario-seeds.json` 的 static-seed-only、unmeasured 边界，也不改变这一事实。
