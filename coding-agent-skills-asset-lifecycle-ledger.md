...
2. 仓库没有 outcome runner、immutable task harness、模型矩阵、成本采集器或可核验 host execution substrate；不能凭空实现或宣称 A/B。
