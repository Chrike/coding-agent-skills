...
- the checked-in project-scoped custom Agent definition/runtime target under `.claude/agents/`
...
This repository currently keeps its only checked-in custom Agent definition at `.claude/agents/fresh-completion-verifier.md`; that file is also the project-scoped runtime target. Invoke it independently with `claude --agent fresh-completion-verifier`. There is no separate top-level `agents/` mirror or installer. A version-specific local Workflow probe accepted an `agentType` option and dispatched the built-in `Explore` type; a probe with `agentType: "fresh-completion-verifier"` failed before worker execution because that project custom Agent was not in that run's Workflow registry. The current public Workflow documentation (https://code.claude.com/docs/en/workflows) shows generic `agent(prompt, options)` examples but does not establish `agentType` or project saved-Agent loading as a supported stable contract, so the status remains `unknown` and the adaptive workflow keeps its inline investigator and verifier prompts.
