...
当前仓库唯一已跟踪的自定义 Agent 定义位于 `.claude/agents/fresh-completion-verifier.md`，该文件同时就是项目级运行时落点；没有单独的顶层 `agents/` 镜像或 installer。可以使用 `claude --agent fresh-completion-verifier` 独立调用。一次版本相关的本地 Workflow probe 接受了 `agentType` 选项并调度了内置 `Explore` 类型；使用 `agentType: "fresh-completion-verifier"` 的 probe 则在 worker 执行前失败，报告该项目 Agent 不在本次 Workflow registry 中。当前公开 Workflow 文档（https://code.claude.com/docs/en/workflows）展示的是通用 `agent(prompt, options)` 示例，没有确认 `agentType` 或项目级 saved-Agent 加载是稳定支持的契约，因此状态仍为 `unknown`，当前 adaptive workflow 继续保留内联 investigator 和 verifier prompt。
