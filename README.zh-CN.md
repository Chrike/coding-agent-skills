...
- 位于 `experiments/` 下的显式选择 frozen Lab workflow 源码
- 位于 `profiles/` 下的非运行时 profile 与分发元数据
- 位于 `governance/` 与 `archive/` 下的非运行时生命周期治理边界
- 位于 `tests/` 下的维护与验证材料
...
- `experiments/` 承载显式选择的 frozen Lab workflow 源码，不是宿主发现目录，也不参与普通 skill 路由。
...
从 `profiles/manifest.json` 选择并安装非运行时分发 profile；如果需要更窄的集合，也可以手工复制源码目录。

profile 有 `fast`、`quality` 和 `lab` 三个选项。`quality` 是策略默认值；`lab` 仅显式启用，并包含 frozen adaptive pilot。profile 只选择分发内容，不改变自然语言路由、宿主策略、provider 身份或 outcome 证据。

使用显式目标根目录调用无依赖安装器；默认只做 dry-run：
...
只有 `--apply` 才会写入；替换不同的已拥有文件必须显式使用 `--force`。安装器不会删除、清理、写入 `CLAUDE.md`、修改宿主设置或安装 tests/evals/governance 文件，也不能证明宿主强制隔离或 provider/runtime 身份。

在本仓库中，`skills/`、`prompts/` 与 `experiments/` 都是源码目录，而不是宿主运行时路径。安装器将选中的技能复制到显式的 `<target-root>/skills/`，在 `lab` profile 下将 workflow 复制到 `<target-root>/workflows/`。
...
`prompts/CLAUDE.fragment.md` 仍需手工组装到宿主指令体系；安装器不会覆盖 `CLAUDE.md`。

Saved workflow 采用显式选择：将审阅过的源码文件复制到一个明确选择的落点，再调用其已安装名称。它不替代 skill 路由，也不会因为任务长或文件多而自动激活。

将 `tests/` 保留为维护与验证材料，而不是运行时技能。不要将 `tests/`、`profiles/`、`governance/` 或 `archive/` 复制到运行时安装目录中。
...
- `experiments/` 存放显式 frozen Lab 源码；调用前将审阅过的 workflow 复制到 `.claude/workflows/` 或 `~/.claude/workflows/`。
- `profiles/` 与 `scripts/install-profile.js` 存放非运行时分发元数据和显式复制工具，不定义路由。
- `governance/` 与 `archive/` 存放非运行时生命周期边界，不参与安装。
...
- `experiments/` 存放面向有界、会话内程序化执行试点的显式 frozen Lab 源码；它不是普通 skill 路由层，也不是 outcome 证据。
