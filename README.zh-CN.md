...
- 位于 `.claude/agents/` 下的项目级自定义 Agent 定义和运行时落点
- 位于 `tests/` 下的维护与验证材料
...
只安装实际需要的运行时组件：从 `skills/` 选择技能目录，从 `.claude/agents/` 选择 Agent 定义，并将审阅过的 `workflows/` 源码显式安装到宿主落点。

在本仓库中，`skills/`、`prompts/` 与 `workflows/` 是源码目录，而不是宿主运行时路径；`.claude/agents/` 是项目级 Agent 运行时落点。

已确认的宿主落点：

- Claude Code 运行时技能：项目级 `.claude/skills/` 或用户级 `~/.claude/skills/`
- Claude Code 自定义 Agent：项目级 `.claude/agents/` 或用户级 `~/.claude/agents/`；本工作树在此定义有界 investigator、verifier 和 findings-only reviewer 角色
- Claude Code saved workflow：项目级 `.claude/workflows/` 或用户级 `~/.claude/workflows/`

Saved workflow 采用显式选择：将审阅过的源码文件复制到一个明确选择的落点，再调用其已安装名称。它不替代 skill 路由，也不会因为任务长或文件多而自动激活。`workflows/fresh-findings-review.js` 是一个有界、只读的 findings-only 评审 Workflow：它校验 findings 输出和路径边界，但反馈分诊、修复、完成判断和分支动作仍由 `review-and-finish` 持有。

当前仓库定义的项目级自定义 Agent 运行时目标位于 `.claude/agents/`：`fresh-completion-verifier.md` 负责有界完成验证，`adaptive-evidence-investigator.md` 负责单个只读证据问题，`fresh-findings-reviewer.md` 负责只返回 findings 的独立评审。可以使用 `claude --agent <name>` 独立调用；在安装这份工作树改动后，这些文件就是项目级运行时落点，没有单独的顶层 `agents/` 镜像或 installer。Adaptive Workflow 会先尝试对应的 saved Agent selector；宿主拒绝 selector 时回退到带有相同动态 prompt 的内置只读 `Explore`，因此动态任务状态、schema、validator 和 fallback 所有权仍由 `workflows/adaptive-long-horizon.js` 持有。当前公开 Workflow 文档（https://code.claude.com/docs/en/workflows）展示的是通用 `agent(prompt, options)` 示例，没有确认 `agentType` 或项目级 saved-Agent 加载为稳定契约；selector 支持仍依赖版本，不能作为安全边界。
...
- `tests/` 存放用于维护本套件的路由、Agent 角色、Workflow、Skill 边界及其他维护检查；`tests/routing/`、`tests/agents/`、`tests/workflows/` 和 `tests/skills/` 分别只负责对应层次。
