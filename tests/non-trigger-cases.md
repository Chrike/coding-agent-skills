...
| An adaptive workflow leaf discovered another delegation-worthy question. | `agent-workflow`, nested workflow, new agent tree | return the question to the active workflow instead of adding an orchestration layer |
| An active workflow already has criterion-mapped completion verification. | another review verifier | reuse sufficient verification unless a distinct load-bearing risk remains uncovered |
