...
| Researcher or Brancher receives an incomplete controller brief. | Before any tool call, return a blocked brief naming each missing field and the smallest required next input; the blocked response replaces the normal success schema, so do not emit normal success sections or inspect repository or preferred-candidate evidence or infer the missing boundary. |
| Evaluator receives a brief without actual candidate artifacts or results, hard constraints or evaluation criteria, available judgment evidence, or an expected return contract. | Before evaluating, return only a blocked brief naming each missing item and the smallest required next input; the blocked response replaces the normal evaluation schema, so do not emit normal evaluation sections or a comparative judgment. |
| Brancher's approach was materially affected by repository evidence. | Include an `Evidence basis` section naming the relevant project fact or repository path and the part of the approach it supports; omit the section when repository evidence did not materially affect the approach. |
