...
| The user explicitly authorizes a commit containing multiple unrelated change groups. | Keep the requested scope visible, but split only when the user or acceptance scope permits; do not silently rewrite the requested history or broaden the commit. |
| A commit scope is authorized and logically cohesive, but no universal project gate is established. | Use the relevant available verification evidence without imposing fixed test, lint, type, build, line-count, branch, or toolchain requirements; report checks that were not run. |
| The user supplies a commit message that explains intent. | Preserve it unless it is invalid or unsafe; do not replace it with a path-only message. |
