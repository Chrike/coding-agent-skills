...
| Explicit commit, push, local branch merge, named remote PR merge, current-branch PR preparation or creation, scoped discard, or branch wrap-up action | `finish-branch`; each action authorizes only itself, and local branch merge must not be substituted for remote PR merge or vice versa |
