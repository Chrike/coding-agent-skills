...
| An explicit request to design, add, or audit persistent operational logs, metrics, traces, or alerts, or a concrete observability gap identified by an active owner | `observability-and-instrumentation`; start from on-call questions, choose the minimum project-conforming signal, protect sensitive fields and cardinality, and route runtime backends, monitoring, review, and branch actions to their owners |
| An explicit readiness, rollout, or rollback question for a concrete production release, or a release-specific production evidence gap identified by an active owner | `shipping-and-launch`; build a release packet, distinguish repository from production evidence, report `GO`/`BLOCK`/`UNVERIFIED`, and do not execute deployment, migration, flag, rollback, notification, publication, or Git actions automatically |
...
| Explicit bundled `/security-review` | host security-review workflow for current-diff vulnerability review; do not re-enter `security-and-hardening` as a second diff-review path; use that project owner for explicit threat modeling and trust-boundary analysis |
...
| Explicit planning, roadmap, task breakdown, approach comparison, implementation slices, or a requested implementation with approach/dependency/sequencing/migration/compatibility/scope decisions that cannot be safely inferred | `plan-work`; for an explicit deprecation, replacement, consumer migration, or compatibility transition, read its `deprecation-and-migration` reference without creating a second migration owner |
| Explicit architecture, seams, interfaces, adapters, domain language, prototypes, or an implementation blocked on a non-obvious architecture/ownership/interface/dependency-boundary decision | `design-codebase`; if a durable ADR is requested after the decision, inspect the existing repository convention and use the ADR reference without creating a second documentation owner |
